  ...
  return (
    <PageLayout locale={locale}>
      ...
      <section className="mt-2 mb-8">
        <h2 className="text-xl font-semibold text-ink mb-4">{isAr ? "أسئلة شائعة للوالدين" : "Frequently Asked Questions"}</h2>
      ...
      <table className="w-full text-sm">
        <thead>
          <tr className="border-b border-ink/10">
            <th className="py-2 px-3 text-start text-ink/70 font-medium">{isAr ? "العمر" : "Age"}</th>
            <th className="py-2 px-3 text-start text-ink/70 font-medium">{isAr ? "التركيز" : "Focus"}</th>
            <th className="py-2 px-3 text-start text-ink/70 font-medium">{isAr ? "ماذا تفعل" : "What to do"}</th>
          </tr>
        </thead>
        <tbody className="text-ink/80">
          {stages.map((s) => (
            <tr key={s.ageEn} className="border-b border-ink/5">
              <td className="py-2.5 px-3 text-ink/85 font-medium whitespace-nowrap">{isAr ? s.ageAr : s.ageEn}</td>
...
function ContentEn() {
  return (
    <>
      <h1 className="text-3xl font-semibold text-ink mb-2">What&apos;s the Best Age to Start Teaching Arabic?</h1>
      <p className="text-base text-ink/75 mb-8">A stage-by-stage guide for parents and educators</p>

      <div className="space-y-8 text-sm leading-relaxed text-ink/80">
        <section>
          <h2 className="text-xl font-semibold text-ink mb-3">The Short Answer: As Early as Possible</h2>
        ...
        <section>
          <h2 className="text-xl font-semibold text-ink mb-3">Age-Window Table</h2>
          <StageTable isAr={false} />
        </section>

        <section>
          <h2 className="text-xl font-semibold text-ink mb-3">What Early Exposure Does for the Brain</h2>
        ...
        <section>
          <h2 className="text-xl font-semibold text-ink mb-3">Stage-by-Stage Guide</h2>
          <div className="space-y-4">
            <div className="rounded-xl border border-ink/8 bg-card p-5">
              <h3 className="font-semibold text-ink mb-2">Ages 0–2: Listening</h3>
              <p className="text-ink/80 mb-2">At this age, babies are absorbing the sounds of their environment. They can&apos;t produce language yet, but they&apos;re building a sound library in the brain. Every Arabic conversation, song, or recitation they hear lays down connections.</p>
              <p className="text-ink/80"><strong className="text-ink/90">What to do:</strong> Speak Arabic around your baby. Play Arabic nursery rhymes. Read Arabic board books aloud, even before they understand the words. The rhythm and sounds are what matter now.</p>
            </div>
            <div className="rounded-xl border border-ink/8 bg-card p-5">
              <h3 className="font-semibold text-ink mb-2">Ages 2–4: Sounds &amp; Play</h3>
              <p className="text-ink/80 mb-2">Toddlers learn through cause and effect. They love pressing things and seeing what happens — the perfect age for ArabFingers. The keyboard-smash format lets them explore Arabic letters through play with no expectations.</p>
              <p className="text-ink/80"><strong className="text-ink/90">What to do:</strong> Let your child play freely. Don&apos;t quiz them. Name letters casually as they appear: &quot;Oh, that&apos;s Ba!&quot; Start pointing out Arabic letters on packages, signs, and book covers.</p>
            </div>
            <div className="rounded-xl border border-ink/8 bg-card p-5">
              <h3 className="font-semibold text-ink mb-2">Ages 4–6: Letters</h3>
              <p className="text-ink/80 mb-2">Pre-schoolers can engage in more structured learning. They can name most letters, understand that letters carry sounds, and start tracing shapes. This is when gentle, formal letter work can begin alongside continued play.</p>
              <p className="text-ink/80"><strong className="text-ink/90">What to do:</strong> Trace Arabic letters in sand, salt trays, or on paper. Sing the alphabet. Read simple Arabic words together. Keep play as the main mode of learning.</p>
            </div>
            <div className="rounded-xl border border-ink/8 bg-card p-5">
              <h3 className="font-semibold text-ink mb-2">Ages 6+: Reading</h3>
              <p className="text-ink/80 mb-2">Children with early exposure are ready for reading. They recognise most letters in their connected forms, see how letters join in words, and begin reading short words and sentences — and learning diacritics for precise pronunciation.</p>
              <p className="text-ink/80"><strong className="text-ink/90">What to do:</strong> Move to reading-focused programs. Keep ArabFingers for fun review. Read Arabic children&apos;s books daily. Consider classes or a tutor.</p>
            </div>
          </div>
        </section>

        <section>
          <h2 className="text-xl font-semibold text-ink mb-3">It&apos;s Never Too Late</h2>
...
function ContentAr() {
  return (
    <>
      <h1 className="text-3xl font-semibold text-ink mb-2">ما أفضل عمر لبدء تعليم العربية؟</h1>
      <p className="text-base text-ink/75 mb-8">دليل مرحلة بمرحلة للوالدين والمعلّمين</p>

      <div className="space-y-8 text-sm leading-relaxed text-ink/80">
        <section>
          <h2 className="text-xl font-semibold text-ink mb-3">الجواب المختصر: في أبكر وقت ممكن</h2>
        ...
        <section>
          <h2 className="text-xl font-semibold text-ink mb-3">جدول مراحل العمر</h2>
          <StageTable isAr={true} />
        </section>

        <section>
          <h2 className="text-xl font-semibold text-ink mb-3">ماذا يصنع التعريض المبكّر في الدماغ؟</h2>
        ...
        <section>
          <h2 className="text-xl font-semibold text-ink mb-3">دليل مرحلة بمرحلة</h2>
          <div className="space-y-4">
            <div className="rounded-xl border border-ink/8 bg-card p-5">
              <h3 className="font-semibold text-ink mb-2">٠–٢ سنة: الإصغاء</h3>
              <p className="text-ink/80 mb-2">في هذا العمر يمتصّ الطفل أصوات ما حوله. لا يقدر على النطق بعد، لكنّه يبني مكتبة أصوات في دماغه. وكلّ حديث عربيّ أو نشيد أو تلاوة يسمعها يُرسي وصلات في عقله.</p>
              <p className="text-ink/80"><strong className="text-ink/90">ماذا تفعل:</strong> تحدّث بالعربية حوله، وشغّل الأناشيد العربية، واقرأ الكتب المصوّرة بصوتٍ عالٍ ولو قبل أن يفهم الكلمات؛ فالإيقاع والأصوات هما المهمّ الآن.</p>
            </div>
            <div className="rounded-xl border border-ink/8 bg-card p-5">
              <h3 className="font-semibold text-ink mb-2">٢–٤ سنوات: الأصوات واللعب</h3>
              <p className="text-ink/80 mb-2">الطفل في هذا السنّ يتعلّم بالسبب والنتيجة، ويحبّ أن يضغط فيرى ما يحدث، وهو العمر الأمثل لعرب فنجرز؛ إذ يستكشف الحروف العربية باللعب بلا توقّعات.</p>
              <p className="text-ink/80"><strong className="text-ink/90">ماذا تفعل:</strong> دعه يلعب بحرّية، ولا تختبره، وسمِّ الحروف عَرَضاً كلّما ظهرت: &quot;انظر، هذه باء!&quot;، وابدأ بالإشارة إلى الحروف العربية على العبوات واللافتات وأغلفة الكتب.</p>
            </div>
            <div className="rounded-xl border border-ink/8 bg-card p-5">
              <h3 className="font-semibold text-ink mb-2">٤–٦ سنوات: الحروف</h3>
              <p className="text-ink/80 mb-2">أطفال الروضة يقدرون على تعلّم أكثر تنظيماً؛ يسمّون أكثر الحروف، ويدركون أنّ الحرف يحمل صوتاً، ويبدأون بتتبّع الأشكال. وهنا يبدأ العمل اللطيف على الحروف بجانب استمرار اللعب.</p>
              <p className="text-ink/80"><strong className="text-ink/90">ماذا تفعل:</strong> تتبّعوا الحروف في الرمل أو صينية الملح أو على الورق، وغنّوا نشيد الأبجدية، واقرآ كلمات عربية بسيطة معاً، واجعل اللعب هو الأساس.</p>
            </div>
            <div className="rounded-xl border border-ink/8 bg-card p-5">
              <h3 className="font-semibold text-ink mb-2">٦ سنوات فأكثر: القراءة</h3>
              <p className="text-ink/80 mb-2">الطفل الذي عُرّض مبكّراً صار مهيّأً للقراءة؛ يعرف أكثر الحروف في أشكالها المتّصلة، ويرى كيف تتّصل في الكلمات، ويبدأ بقراءة كلمات وجمل قصيرة، وبتعلّم الحركات لضبط النطق.</p>
              <p className="text-ink/80"><strong className="text-ink/90">ماذا تفعل:</strong> انتقلوا إلى برامج تركّز على القراءة، وأبقِ عرب فنجرز للمراجعة الممتعة، واقرآ كتب الأطفال العربية يومياً، وفكّر في الدروس أو معلّم.</p>
            </div>
          </div>
        </section>

        <section>
          <h2 className="text-xl font-semibold text-ink mb-3">لا يفوت الأوان أبداً</h2>
