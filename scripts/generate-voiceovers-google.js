...
// The site's audio is generated by scripts/generate-voiceovers-eleven.js and tracked
// in the manifest. This script predates that and would silently overwrite every clip
// with a lower-quality voice, so it refuses to run over managed audio.
const MANIFEST = path.join(__dirname, ".tts-manifest.json");
if (!process.argv.includes("--force")) {
  try {
    const tracked = Object.keys(JSON.parse(fs.readFileSync(MANIFEST, "utf8"))).length;
    if (tracked > 0) {
      console.error(`Refusing to run: ${tracked} clips are managed by another generator.`);
      console.error("Overwriting them would downgrade the site's audio.");
      console.error("Use scripts/generate-voiceovers-eleven.js, or pass --force if you mean it.");
      process.exit(1);
    }
  } catch { /* no manifest yet — nothing to protect */ }
}
