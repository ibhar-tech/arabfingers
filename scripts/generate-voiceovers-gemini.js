...
async function generateAll() {
  ...
  for (const course of courseKeys) {
    ...
    for (const scene of scenes) {
      ...
      try {
        ...
        arGenerated = await generateGeminiAudio(scene.ar, "ar", voice, arPath);
        if (arGenerated) totalGenerated++;
      } catch (err) {
      ...
      try {
        ...
        enGenerated = await generateGeminiAudio(scene.en, "en", voice, enPath);
        if (enGenerated) totalGenerated++;
      } catch (err) {
        console.error(`  ❌ Failed Gemini EN Scene ${scene.id}:`, err.message);
      }

      // Respect API rate limits safely: only wait if we actually made a generative API call!
      if (arGenerated || enGenerated) {
  ...
  console.log(`\n🎉 Super success! Successfully generated ${totalGenerated} premium Gemini 2.0 voiceover files!`);
}
